...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		w = TEXTW(tagtext);
		drw_setscheme(drw, (m->tagset[m->seltags] & 1 << i ? tagscheme[i+1] : scheme[SchemeNorm]));
		drw_text(drw, x, 0, w, bh, lrpad / 2, tagtext, urg & 1 << i);
		x += w;
	}
	w = blw = TEXTW(m->ltsymbol);
	drw_setscheme(drw, m->seltagscheme);
	...
	if ((w = m->ww - tw - x) > bh) {
		if (m->sel) {
			...
			if (m->sel->isfloating) {
                                int fw = TEXTW("(f)") - (lrpad / 2);
                                drw_text(drw, x, 0, fw, bh, lrpad / 2, "(f)", 0);
                                x += fw;
                                w = m->ww - tw - x;
                        }
...
view(const Arg *arg)
{
	...
	selmon->lt[selmon->sellt^1] = selmon->pertag->ltidxs[selmon->pertag->curtag][selmon->sellt^1];
        ...
        selmon->seltagscheme = tagscheme[selmon->pertag->curtag];
